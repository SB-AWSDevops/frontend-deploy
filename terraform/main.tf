#creating the frontend infrastructure
...
#configure using ansible with help of terraform null resource
resource "null_resource" "frontend" {
  #whenever the frontend module's instance is created, updated, or destroyed, 
  #this null_resource will be triggered.
  triggers = {
...
resource "aws_ec2_instance_state" "frontend" {
  ...
  state       = "stopped"
  #stop the server only when null resource provisioning is completed, otherwise we should not run this resource
  depends_on = [null_resource.frontend]

}

#take the AMI of the created frontend instance
...
resource "aws_lb_target_group" "frontend" {
  ...
  health_check {
    ...
    matcher             = "200-299" # has to be HTTP between 200-299 or fails
  }
}
...
#create the auto scaling group and attaching the launch template
...
# adding listener rule to the frontend load balancer
